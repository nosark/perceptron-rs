...
impl Perceptron {
    pub fn new(learning_rate: Option<f64>, epochs: Option<u32>, input: &Vec<Iris>) -> Self {
        Perceptron {
            learning_rate: learning_rate.unwrap_or(0.1) as f32,
    ...
    pub fn fit(&mut self, X: &Vec<Iris>, y: &Vec<f32>) -> Self {
        let mut update;
        let mut errors = 0;
    ...
    pub fn predict(&self, wi: usize, X: &Iris) -> f32 {
    ...
    fn net_input(&self, wi: usize, X: &Iris) -> f32 {
        let output;
        output = self.dot(wi, X) + self.bias;
        output
    }

    fn update_weights(&mut self, wi: usize, X: &Iris, update: f32) {
    ...
    fn dot(&self, wi: usize, X: &Iris) -> f32 {
        let mut output = 0.0;
        for i in 0..X.len() {
